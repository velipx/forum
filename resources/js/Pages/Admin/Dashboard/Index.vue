        ...
        <div class="hidden md:flex md:w-80 md:flex-col md:fixed md:inset-y-0">
            ...
            <div class="flex-1 flex flex-col min-h-0 bg-slate-900">
                <div class="flex items-center h-16 flex-shrink-0 px-4 bg-slate-800">
                    <h1 class="text-white text-xl">asd</h1>
                </div>
                <div class="flex-1 flex flex-col overflow-y-auto pt-5">
                    <nav class="flex-1 px-6 py-4 space-y-1">
                        <template v-for="item in navigation" :key="item.name">
                            <div v-if="!item.children">
                                <a :href="item.href" :class="[item.current ? 'bg-slate-800/40 text-indigo-500' : 'text-white hover:bg-slate-800/40', 'group flex items-center px-2 py-2 text-sm font-semibold rounded-md']">
